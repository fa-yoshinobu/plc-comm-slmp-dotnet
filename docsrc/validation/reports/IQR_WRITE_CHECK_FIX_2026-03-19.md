...
```bash
dotnet run --project samples/PlcComm.Slmp.Cli -- random-check --host 192.168.250.100 --port 1025 --transport tcp --target SELF --write-check
dotnet run --project samples/PlcComm.Slmp.Cli -- block-check --host 192.168.250.100 --port 1025 --transport tcp --target SELF --write-check
```
