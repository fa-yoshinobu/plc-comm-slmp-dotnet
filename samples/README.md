...
```powershell
dotnet run --project samples/PlcComm.Slmp.HighLevelSample -- 192.168.250.100 1025 melsec:iq-r tcp SELF
dotnet run --project samples/PlcComm.Slmp.PollingReconnectSample -- 192.168.250.100 1025 melsec:iq-r D100 U tcp SELF 1
...
| --- | --- | --- |
| [PlcComm.Slmp.HighLevelSample](PlcComm.Slmp.HighLevelSample/) | Walks through the main high-level APIs in one program. | Shows `SlmpClientFactory.OpenAndConnectAsync`, typed reads and writes, block reads, bit-in-word updates, named value sets, and polling. |
...
- The CLI sample includes `device-range-catalog` for user-selected PLC profile
  ...
  `dotnet run --project samples/PlcComm.Slmp.Cli -- device-range-catalog --plc-profile melsec:iq-f --host 192.168.250.100 --port 1025 --transport tcp --target SELF`
- The CLI sample includes `extendeddevice-coverage` for Extended Specification
  ...
  `dotnet run --project samples/PlcComm.Slmp.Cli -- extendeddevice-coverage --host 192.168.250.100 --port 1025 --transport tcp --target SELF --plc-profile melsec:iq-r --device U3E0\G10 --points 1 --write-check`
...
| Command | Example |
| --- | --- |
| Connection check | `dotnet run --project samples/PlcComm.Slmp.Cli -- connection-check --plc-profile melsec:iq-r --host 192.168.250.100 --port 1025 --transport tcp --target SELF` |
| Device range catalog | `dotnet run --project samples/PlcComm.Slmp.Cli -- device-range-catalog --plc-profile melsec:iq-r --host 192.168.250.100 --port 1025 --transport tcp --target SELF` |
